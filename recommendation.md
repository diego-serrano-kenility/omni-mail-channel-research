...
**Justificacion**:
1. **Zero cambios DNS** - El email sale desde la infra que el cliente ya usa
...
4. **SMTP Directo** (6.7/10) - Opcion valida para multi-tenant futuro. Zero DNS, zero costo, DMARC nativo. Limitado por tracking DIY y gestion de credenciales
...
**Implementar con Amazon SES como Opcion A**, aprovechando la experiencia del equipo con AWS, los contratos vigentes y el costo significativamente menor a escala. Mantener **SendGrid como Opcion B** si se prefiere un setup mas rapido con menos desarrollo DIY (inbound parse nativo vs pipeline S3/SNS). **Gmail API como Opcion C (fallback)** si la coordinacion de cambios DNS resulta muy lenta. Para la **vision multi-tenant a futuro**, evaluar el enfoque SMTP Directo (Opcion D) como segunda fase.
