...
|---|---|---|---|---|---|
| **Inbound parsing** | DIY (S3 + SNS + Lambda) | Nativo (webhook POST) | Nativo (Pub/Sub + history) | Forwarding + provider | Nativo (webhook POST) |
...
| **Sin control DNS posible** | No | No | **Si** | **Si** | Parcial (DKIM ya esta) |
...
| **IAM auth (sin API keys)** | **Si** | No | No | No | No |
| **SNS/SQS integracion** | **Si** | No | No | No | No |
| **Terraform/IaC** | **Si** (nativo AWS) | Parcial (provider TF) | No | No | No |
| **Ya usa la infraestructura** | **Si** (ya en AWS) | No | No | No | No |
...
| **Tiempo estimado POC** | 2-4 dias | 1-2 dias | 2-4 dias | 2-4 dias | 1-2 dias |
...
| Criterio | Amazon SES | SendGrid | Gmail API | SMTP Directo | Brevo |
|---|---|---|---|---|---|
| **Free tier envio** | 62,000/mes (desde EC2/ECS) | Trial 60 dias (no permanente) | 2,000/dia/usuario | N/A (usa cuota del cliente) | 300/dia (permanente) |
| **Limite maximo diario** | 50,000/dia (ajustable) | Segun plan | **2,000/dia** (hard cap) | 2,000-10,000/dia (segun proveedor SMTP) | Segun plan |
| **Escalabilidad** | Muy alta (millones) | Alta (millones) | **Baja** (2,000/dia) | Limitada por SMTP del cliente | Alta (segun plan) |
| **Tamanio max mensaje** | 10 MB | 30 MB | 25 MB (50 MB upload) | 25 MB (Gmail) / 25 MB (M365) | 20 MB |
...
- **Debilidad**: Sin tracking nativo, limite 2,000/dia, requiere Workspace admin
